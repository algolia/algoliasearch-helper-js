...
test('Should throw if there is already a refinement', function() {
  ...
  expect(state1.addHierarchicalFacetRefinement.bind(state1, 'categories', 'men')).toThrow();
});

test('Should throw if the facet is not defined', function() {
  var state0 = SearchParameters.make({});

  expect(state0.addHierarchicalFacetRefinement.bind(state0, 'categories', 'men')).toThrow();
});
