...
during the search cycle with events:
 ...
 - `noMoreSearch`: when there is no more on going searches
...
Finally, you can retrieve if there is an on-going search with `isSearchPending` or by
listening to the `noMoreSearch` event.

{{> jsdoc jsdoc/helper/search}}
{{> jsdoc jsdoc/helper/searchOnce}}
{{> jsdoc jsdoc/helper/searchForFacetValues}}
{{> jsdoc jsdoc/helper/isSearchPending}}
